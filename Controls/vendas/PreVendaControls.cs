...
namespace CasaMendes.Classes.Geral
{
    class Cl_PreVenda //: Propriedades.Cl_pPreVenda
    {
        ...
        //public static void Receber(string _CodigoDoCliente, decimal Dinheiro, decimal Troco)
        //{
        //    if (oDados.Params.Count != 0) oDados.Params.Clear();
        ...
        //    string sSql = "UPDATE tPreVendas SET DataDaVenda = @DataDaVenda, TipoDeVenda = 'A VISTA', Dinheiro = @Dinheiro, Troco = @Troco WHERE CodigoDoCliente = @CodigoDoCliente AND TipoDeVenda = 'ANOTAR'";
        //    oDados.ExecutarQueryVendas(sSql, true);
        //}
        ...
        //        string sSql = "SELECT tPreVendas.CodigoDaVenda, tProdutos.Produto, tPreVendas.DataDaVenda as [Data venda], (tPreVendas.Quantidade) as Qtde, (((tPreVendas.Quantidade * tProdutos.PrecoDeVenda)) - (tPreVendas.DescontoAplicado)) as [Vendas], (tPreVendas.Quantidade * tProdutos.PrecoUnitario) as [Custo], ((((tPreVendas.Quantidade * tProdutos.PrecoDeVenda)) - (tPreVendas.DescontoAplicado)) - (tPreVendas.Quantidade * tProdutos.PrecoUnitario)) as Lucro FROM [tPreVendas] inner join [tProdutos] on [tPreVendas].[CodigoDeBarras] = [tProdutos].[CodigoDeBarras]  where (DataDaVenda >= '" + string.Format(inicio, "yyyy-mm-dd") + "') AND ( DataDaVenda <= '" + string.Format(intervalo, "yyyy-mm-dd") + "') AND ( TipoDeVenda =  'A VISTA') ORDER BY tProdutos.Produto";

        //        if ((inicio == "") || (intervalo == ""))
        //        {
        //            sSql = "SELECT tPreVendas.CodigoDaVenda, tProdutos.Produto, tPreVendas.DataDaVenda as [Data da venda], (tPreVendas.Quantidade) as Qtde, (tPreVendas.Quantidade * tProdutos.PrecoDeVenda) as [Vendas], (tPreVendas.Quantidade * tProdutos.PrecoUnitario) as [Custo], ((tPreVendas.Quantidade * tProdutos.PrecoDeVenda) - (tPreVendas.Quantidade * tProdutos.PrecoUnitario)) as Lucro FROM [tPreVendas] inner join [tProdutos] on [tPreVendas].[CodigoDeBarras] = [tProdutos].[CodigoDeBarras] ORDER BY tProdutos.Produto";
        //        }
