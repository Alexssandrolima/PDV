...
namespace CasaMendes.Classes.ADL
{
    ...
    public static class Cl_Dados
    {
        ...
        #region Variáveis
        //======================================================================================================
        private static SqlCeConnection oConexao;
        private static SqlCeCommand oComando;
        private static SqlCeDataAdapter oDa;
        private static SqlCeDataReader oDr;
        private static SqlCeTransaction oTransacao;
        private static DataTable oDt;

        //======================================================================================================
        public static List<SqlCeParameter> Params = null;
        private static string sConexao; //= "Data Source = " + clsGlobal.ValidarDiretorio(sPath, "loja.sdf");;
        public static string sSql;
        private static bool Retorno = false;
        private readonly static string sPath = clsGlobal.ValidarDiretorio(clsGlobal.sCaminho, "CMDB");
        ...
        {
            sConexao = "Data Source = " + clsGlobal.ValidarDiretorio(sPath, "loja.sdf");

            if (Params == null) Params = new List<SqlCeParameter>();

            if (!Directory.Exists(sPath))
            {
                Directory.CreateDirectory(sPath);

                if (!File.Exists(clsGlobal.ValidarDiretorio(sPath + "loja.sdf")))
                {
                    CriarDb();
                }
            }
            else { AtualizarSqlCeEngine(); }
        }

        public static void AtualizarSqlCeEngine()
        {
            try
            {
                ...
                {
                    //motor.Shrink
                    motor.Upgrade(); //Atuaaliza o motor de dados.
                    motor.Dispose();
                }
            }
            catch { }
        ...
        private static void CriarDb()
        {
            try
            {
                ...
                using (SqlCeEngine motor = new SqlCeEngine(sConexao))
                {
                    ...
                    oConexao = new SqlCeConnection(sConexao);
                    oConexao.Open();
                    ...
                    oComando = new SqlCeCommand(sSql, oConexao);
        ...
        public static DataSet CarregarDataset(string sSql, string sTabela)
        {
            try
            {
                using (oConexao = new SqlCeConnection(sConexao))
                {
                    oConexao.Open();
                    DataSet oDs = new DataSet(sTabela);
                    oDa = new SqlCeDataAdapter
                    {
                        SelectCommand = new SqlCeCommand(sSql, oConexao)
                    };
                    oDs.Clear();
        ...
        public static string RetornoEscalarDeUmRegistro(string sSql)
        {
            ...
            try
            {
                using (oConexao = new SqlCeConnection(sConexao))
                {
                    ...
                    if (Params.Count != 0)
                    {
                        foreach (SqlCeParameter p in Params)
                        {
        ...
        public static ArrayList ListarDados(string sSql, string campo = null, string filto = null)
        {
            ...
            try
            {
                using (oConexao = new SqlCeConnection(sConexao))
                {
                    ...
                    string commandText = sSql;
                    oComando = new SqlCeCommand(commandText, oConexao);
                    ...
                    //    string x = oDr.GetValue(c).ToString();
                    //    if (x == null) continue;
                    //    arr.Add(x);
                    ////arr.Add((string)oDr.GetSqlString(oDr.GetValue(c)));
                    //    c++;
                //}
                //}
            }
...
//}
        ...
        public static DataTable ListarDescontos(string sSql)
        {
            try
            {
                using (oConexao = new SqlCeConnection(sConexao))
                {
                    ...
                    SqlCeCommand oComando = new SqlCeCommand(sSql, oConexao);
        ...
        public static void AdicionarParametro(string Name, Object Value)
        {
            ...
            SqlCeParameter NewParam = new SqlCeParameter(Name, Value);
            Params.Add(NewParam);
        }
        ...
        //======================================================================================================
        public static void ExecutarQuery(string _sSql)
        {
            try
            {
                oConexao = new SqlCeConnection(sConexao);
                ...
                oComando = new SqlCeCommand(_sSql, oConexao);
                oTransacao = oConexao.BeginTransaction();
                oComando.Transaction = oTransacao;
        ...
        private static bool ExecutarQuery(bool ComParametro)
        {
            try
            {
                ...
                if (oConexao.State == ConnectionState.Open) { oConexao.Close(); }
                oConexao.Open();
                oComando = new SqlCeCommand(sSql.Trim().ToLower(), oConexao);
                oTransacao = oConexao.BeginTransaction();
                oComando.Transaction = oTransacao;
        ...
        public static void Excluir(string Tabela, string ChavePrimaria, string Filtro = "*")
        {
            ...
            if (Resultado == DialogResult.Yes)
            {
                if (Filtro == "*")
                {
                    sSql = string.Concat("DELETE From [", Tabela + "]");
                }
                else
                {
                    sSql = string.Concat("DELETE ", Tabela, " WHERE ", ChavePrimaria, " = '", Filtro, "';");
                }
                try
                {
                    if (ExecutarQuery(true))
